...
import { CREATE_DELIVERY } from "../../../actions/types";
...
function AssignDeliveryCreateForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    const dataValue = {
      ...
      logisticsPartnerState: logisticsPartnerState,
      logisticsPartnerCountry: logisticsPartnerCountry,
    };

    if (!dataValue) {
      props.handleFailedSnackbar(
        "Please select the Order you want to assign to a logistics partner"
      );
      setLoading(false);
      return;
    }
    ...
    if (dataValue) {
      const editForm = async () => {
        ...
        if (conResponse.data.results > 0) {
          props.handleFailedSnackbar(
            "This order had already been assigned to a logistic partner. To reassign, edit the existing assignment"
          );
          setLoading(false);
          return;
        }
        ...
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/deliveries`, dataValue);
      ...
      <form id="assignDeliveryCreateForm" className={classes.formStyles}>
        ...
        >
          <FormLabel
            ...
            component="legend"
          >
            <Typography variant="h5">Assign Order for Delivery</Typography>
          </FormLabel>
        </Grid>
        <Box
          sx={{
