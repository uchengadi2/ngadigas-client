...
import { EDIT_RETURNDELIVERY } from "../../../actions/types";
...
const renderDeliveryReturnDateField = ({
  input,
  label,
  ...
  return (
    <TextField
      ...
      helperText="Delivery Return Date"
      variant="outlined"
      label={label}
      id={input.name}
...
function ReturnDeliveryEditForm(props) {
  const { params } = props;
  ...
  const deliveryReturnDate = params.deliveryReturnedDate
    ? new Date(params.deliveryReturnedDate).toISOString().slice(0, 10)
    : "";

  const buttonContent = () => {
    return <React.Fragment>Return Delivery</React.Fragment>;
  };

  const onSubmit = (formValues) => {
    setLoading(true);

    if (!formValues["deliveryReturnedDate"]) {
      props.handleFailedSnackbar(
        "Please select the Delivery return date and try again"
      );
      setLoading(false);
      return;
    }
    ...
    const dataValue = {
      status: "returned",
      deliveryReturnedDate: formValues["deliveryReturnedDate"]
        ? formValues["deliveryReturnedDate"]
        : params.deliveryReturnedDate,
    };
  ...
  const deliveryReturnedDate = params.deliveryReturnedDate
    ? new Date(params.deliveryReturnedDate).toISOString().slice(0, 10)
    : "";
  ...
  return (
    <div>
      <form id="returnDeliveryEditForm" className={classes.formStyles}>
        <Grid
          item
          container
        ...
        >
          <FormLabel
            ...
            component="legend"
          >
            <Typography variant="h5">On Returned Delivery</Typography>
          </FormLabel>
        </Grid>
        <Box
          sx={{
        ...
        >
          ...
          <Field
            label=""
            id="deliveryReturnedDate"
            name="deliveryReturnedDate"
            defaultValue={deliveryReturnedDate}
            type="date"
            component={renderDeliveryReturnDateField}
            style={{ marginTop: 10 }}
          />
...
export default reduxForm({
  form: "returnDeliveryEditForm",
})(ReturnDeliveryEditForm);
