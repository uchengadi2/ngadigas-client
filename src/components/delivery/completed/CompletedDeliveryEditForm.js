...
import { EDIT_COMPLETEDDELIVERY } from "../../../actions/types";
...
const renderDeliveryCompletedDateField = ({
  input,
  label,
  ...
  return (
    <TextField
      ...
      helperText="Delivery Completed Date"
      variant="outlined"
      label={label}
      id={input.name}
...
function CompletedDeliveryEditForm(props) {
  const { params } = props;
  ...
  const deliveryCompletedDate = params.deliveryCompletedDate
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["deliveryCompletedDate"]) {
      props.handleFailedSnackbar(
        "Please select the Delivery completed date and try again"
      );
      setLoading(false);
      return;
    }
    ...
    const dataValue = {
      status: "fullfilled",
      deliveryCompletedDate: formValues["deliveryCompletedDate"]
        ? formValues["deliveryCompletedDate"]
        : params.deliveryCompletedDate,
    };
  ...
  return (
    <div>
      <form id="completedDeliveryEditForm" className={classes.formStyles}>
        <Grid
          item
          container
        ...
        >
          ...
          <Field
            ...
            name="deliveryCompletedDate"
            defaultValue={deliveryCompletedDate}
            type="date"
            component={renderDeliveryCompletedDateField}
            style={{ marginTop: 10 }}
          />
...
export default reduxForm({
  form: "completedDeliveryEditForm",
