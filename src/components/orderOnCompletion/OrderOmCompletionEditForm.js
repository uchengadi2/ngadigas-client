...
import { EDIT_COMPLETED_ORDER } from "../../actions/types";
...
function OrderOmCompletionEditForm(props) {
  const { params } = props;
  ...
  const [vendor, setVendor] = useState(params.category);
  ...
  const [onTransitOrder, setOnTransitOrder] = useState(params.onTransitOrder);
  ...
  const buttonContent = () => {
    return <React.Fragment> Update Fullfilled Order</React.Fragment>;
  };
    ...
    if (formValues) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/orderoncompletions/${params.id}`,
          formValues
        );

        if (response.data.status === "success") {
          dispatch({
            type: EDIT_COMPLETED_ORDER,
          ...
          props.handleSuccessfulEditSnackbar(
            `${response.data.data.data.refNumber} fullfilled Order is updated successfully!!!`
          );
  ...
  const dateFullfilled = params.dateFullfilled
    ? new Date(params.dateFullfilled).toISOString().slice(0, 10)
    : "";
        ...
        <Grid container direction="row" style={{ marginTop: 10 }}>
          <Grid item style={{ width: "60%" }}>
            <Field
              ...
              id="recieverName"
              name="recieverName"
              defaultValue={params.recieverName}
              type="text"
          ...
          <Grid item style={{ width: "39%", marginLeft: 5, marginTop: 0 }}>
            <Field
              ...
              defaultValue={params.recieverPhoneNumber}
              type="text"
...
export default reduxForm({
  form: "orderOmCompletionEditForm",
