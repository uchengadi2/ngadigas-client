...
import { fetchOrdersForDelivery } from "../../actions";
...
import OrderForDeliveryEditForm from "./OrderForDeliveryEditForm";

class OrdersForDeliveryList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editOpen: false,
  ...
  componentDidMount() {
    this.props.fetchOrdersForDelivery(this.props.token, this.props.status);
  }
          ...
          <strong>
            ...
            <EditRoundedIcon
              ...
              onClick={() => [
                ...
                history.push(`/orders/ordersfordelivery/edit/${params.id}`),
              ]}
            />
          </strong>
        ),
...
const mapStateToProps = (state) => {
  return { orders: Object.values(state.ordersForDelivery) };
};

export default connect(mapStateToProps, { fetchOrdersForDelivery })(
  OrdersForDeliveryList
);
