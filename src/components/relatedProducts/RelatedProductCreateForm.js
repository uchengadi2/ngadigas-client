...
import { CREATE_RELATEDPRODUCT } from "../../actions/types";
...
function RelatedProductCreateForm(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
      const response = await api.get("/products", {
        params: { productVendor: props.vendorId },
      });
      const workingData = response.data.data.data;
      workingData.map((product) => {
        allData.push({
  ...
  const onSubmit = (formValues) => {
    ...
    if (data) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/relatedproducts`, data);

        if (response.data.status === "success") {
          dispatch({
            type: CREATE_RELATEDPRODUCT,
...
export default reduxForm({
  form: "relatedProductCreateForm",
})(RelatedProductCreateForm);
