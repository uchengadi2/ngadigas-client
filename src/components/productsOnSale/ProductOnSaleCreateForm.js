...
import { CREATE_PRODUCTONSALE } from "../../actions/types";
...
function ProductOnSaleCreateForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    if (data) {
      const createForm = async () => {
        api.defaults.headers.common["Authorization"] = `Bearer ${props.token}`;
        const response = await api.post(`/productsonsale`, data);

        if (response.data.status === "success") {
          dispatch({
            type: CREATE_PRODUCTONSALE,
