...
import { EDIT_REMITTANCE } from "../../actions/types";
...
function RemittanceEditForm(props) {
  ...
  const onSubmit = (formValues) => {
    ...
    if (!formValues["amountRemitted"] && !params.amountRemitted) {
    ...
    if (!formValues["dateRemitted"] && !params.dateRemitted) {
      props.handleFailedSnackbar(
    ...
    if (dataValue) {
      const editForm = async () => {
        ...
        const response = await api.patch(
          `/remittances/${params.id}`,
          dataValue
        );

        if (response.data.status === "success") {
          dispatch({
            type: EDIT_REMITTANCE,
  ...
  const remittanceDate = params.dateRemitted
    ? new Date(params.dateRemitted).toISOString().slice(0, 10)
    : "";

  return (
    <div>
      <form id="remittanceEditForm" className={classes.formStyles}>
        <Grid
          item
          container
          ...
          <Grid container direction="row" style={{ marginTop: 20 }}>
            <Grid item style={{ width: 250 }}>
              <Field
                ...
                id="amountRemitted"
                name="amountRemitted"
                defaultValue={params.amountRemitted}
                type="text"
