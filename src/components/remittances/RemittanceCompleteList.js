...
import { fetchCompleteRemittances } from "../../actions";
import DataGridContainer from "../DataGridContainer";
import RemittanceEdit from "./RemittanceEdit";
import RemittanceDelete from "./RemittanceDelete";

class RemittanceCompleteList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editOpen: false,
  ...
  componentDidMount() {
    this.props.fetchCompleteRemittances(this.props.token, this.props.status);
  }
          ...
          <strong>
            ...
            <EditRoundedIcon
              onClick={() => [
                ...
                history.push(`/remittances/complete/edit/${params.id}`),
              ]}
            />
          </strong>
        ),
          ...
          <strong>
            ...
            <DeleteRoundedIcon
              ...
              onClick={() => [
                ...
                history.push(`/remittances/complete/delete/${params.id}`),
              ]}
            />
          </strong>
        ),
  ...
  render() {
    return (
      <>
        ...
        {this.renderCompleteRemittancesList()}
      </>
    );
  }
}
...
const mapStateToProps = (state) => {
  return { remittances: Object.values(state.remittanceComplete) };
};

export default connect(mapStateToProps, { fetchCompleteRemittances })(
  RemittanceCompleteList
);
